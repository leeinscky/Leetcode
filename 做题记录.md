...
| 数据结构类型 | 计算尺寸大小 | 添加元素 | 弹出元素 | 
|:----:|:----:|:----:|:----:|
| 队列 queue | size() | offer() | poll() |
| 栈 stack |  | push() | pop() |
...
不同之处: add() 方法在添加失败（比如队列已满）时会报 一些运行时错误；
...
2. 输入：一个字符串：ababa。a：出现3次；b：出现2次；ab：出现2次；ba：出现2次；aba：出现1次；abab：出现1次；ababa：1次 输出：一共3+2+2+2+1+1=11次 
