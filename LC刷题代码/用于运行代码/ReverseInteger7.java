...
class ReverseInteger7 {
    public int reverse(int x) {
        ...
        while (x != 0) {
            ...
            /*
             * push 函数最终输出的rev的表达式就在这下面那行，也就是用这个语句之前的rev进行“rev * 10 + pop”的操作， 
             * rev * 10: 最终结果 - 个位数 pop:最终结果的个位数 
             * 因此为了防止栈溢出，我们要满足: Integer.MIN_VALUE < rev * 10 + pop < Integer.MAX_VALUE
             */
            rev = rev * 10 + pop; 
    ...
    public static void main(String[] args){
        ReverseInteger7 sol = new ReverseInteger7(); // 在静态方法内部是不可以直接访问非静态成员的。
